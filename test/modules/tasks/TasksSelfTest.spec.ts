...
describe('Tasks Module Self-Test', () => {
  ...
  it('should validate Tasks module configuration', async () => {
    const result = await testRunner.runSingleModuleTest('tasks')
    ...
    if (result.config) {
      expect(result.config.name).toBe('Task Management')
      expect(result.config.routePrefix).toBe('tasks')

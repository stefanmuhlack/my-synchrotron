...
describe('Checklists Module Self-Test', () => {
  ...
  it('should validate Checklists module configuration', async () => {
    const result = await testRunner.runSingleModuleTest('checklists')
    ...
    if (result.config) {
      expect(result.config.name).toBe('Checklisten')
      expect(result.config.routePrefix).toBe('checklists')
      expect(result.config.rolesAllowed).toContain('user')
